class IngredientConvertions {
    ...
    static amountToEquivalent(ingredient, newValue) {
        ...
        const newEquivalent = (newValue / (equivalent * portionUnit)).toFixed(
            1
        );
        console.log(newValue, equivalent, portionUnit, newEquivalent);

        return {
            equivalent: newEquivalent,
            measure: `${this.formatAmount(newValue / portionUnit)} ${
                ingredient.unit
            }`,
        };
    }
    ...
    static equivalentToAmount(ingredient, newValue) {
        ...
        return {
            amount: newAmount,
            measure: `${this.formatAmount(newAmount / portionUnit)} ${
                ingredient.unit
            }`,
        };
    }
    ...
    static formatAmount(amount) {
        ...
        const fraction =
            commonFractions[closestFraction] ||
            this.convertToFraction(decimalPart);
    ...
    static convertToFraction(decimal) {
        ...
        return `${Math.ceil(numerator / divisor)}/${Math.ceil(
            denominator / divisor
        )}`;
    }
}
